...
if localtest:
    #saveimg=True
    print('mk_plots' + mk_plots)
    saveimg=mk_plots!='false'
    print('saveimg: ' + str(saveimg))
...
if not localtest:
    ...
    USE_YAML=os.environ.get("USE_YAML")=='true'
    f=fs.open(userMyFilesdir+'/'+YAML_FILE, "r")
    fs.download(userMyFilesdir+'/'+YAML_FILE,'config.yaml')
    f=fs.open('./config.yaml', "r")
...
if USE_YAML:
    ...
    Dupper=-config['options']['positioning']['upper_limit'] # upper beach s.depths >= Dupper) & (s.depths <= Dlower
    Dlower=-config['options']['positioning']['lower_limit'] # Lower Beach
else: #load from environment variables

    config={'options':{}}
    config['options']['area']=dict.fromkeys(['min_lon','max_lon','min_lat','max_lat'])
    config['options']['Plant Physiology']=dict.fromkeys(['stem_diameter','drag_coefficient','canopy_height'])
    config['options']['positioning']=dict.fromkeys(['upper_limit','lower_limit'])

    SLR=0 #config['options']['scenario']['SLR']
    # Define Area
    config['options']['area']['min_lon']=LONmin=float(os.environ.get("LON_MIN"))
    config['options']['area']['max_lon']=LONmax=float(os.environ.get("LON_MAX"))
    config['options']['area']['min_lat']=LATmin=float(os.environ.get("LAT_MIN"))
    config['options']['area']['max_lat']=LATmax=float(os.environ.get("LAT_MAX"))
    bbox=np.asarray( [[LONmin,LATmin],[LONmax,LATmax]])

    # define plant characteristics
    config['options']['Plant Physiology']['stem_diameter']=Dstem=0.014
    config['options']['Plant Physiology']['drag_coefficient']=Cdrag=1
    config['options']['Plant Physiology']['canopy_height']=Hcanopy=0.02

    # def # assign nbs
    # invert postiv downard definitions
    Dupper=-float(os.environ.get("UPPER_LIMIT"))
    Dlower=-float(os.environ.get("LOWER_LIMIT"))
    
    config['options']['positioning']['upper_limit']=-Dupper
    config['options']['positioning']['lower_limit']=-Dlower
##################################
...
if saveimg:
    ...
    figname='SG{:.1f}-{:.1f}_{:s}.png'.format(Dupper,Dlower,'SeagrassBeachCoverage')
...
now=now[:now.index('.')].replace(' ','_').replace(':','_')
outname=os.path.join(userdir,'nbsconfig_'+now+'.yaml')
...
with open(lslog ,'a') as f:
    f.write('ls gives:' + str(os.listdir('./')))
    f.write('fs-ls gives:' + str(fs.listdir(userMyFilesdir)))

with open(log ,'w') as f:
    f.write('yaml file: ')    
    f.write(YAML_FILE)
    f.write(' env variables: ')    
    f.write(str(os.environ.get))
